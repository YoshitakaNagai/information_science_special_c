...
if __name__ == '__main__':
    ...
    #for i , j in enumerate(cell):
        #learn_step += len(cell[i].position_log)
        #print("cell[",i,"].position.size()", learn_step)
    for idx, cell_ in enumerate(cell):
        color = "o"+colorlist[idx]
        plt.plot(cell[idx].position_log[:,0], cell[idx].position_log[:,1], color, label="cell_division")
    plt.legend()
    plt.show()
...
"""    
    ...
    for idx, diff_ in enumerate(diff):
        color = colorlist[idx]
        plt.plot(np.arange(diff[idx].Diff_roop, diff[idx].Diff_log, color, label="diff_process")
